      ...
      <div className="grid gap-6 lg:grid-cols-[0.42fr_0.58fr]">
        <Card title="Biểu mẫu tạo lớp học phần" description="Nhập thông tin cơ bản để tạo section mới">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Input label="Phòng học" value={form.room} onChange={(event) => setForm((value) => ({ ...value, room: event.target.value }))} />
      ...
      <div className="grid gap-6 lg:grid-cols-[0.52fr_0.48fr]">
        ...
        {selected ? (
          <Card title="Cập nhật phòng và lịch" description="Thay đổi sẽ được kiểm tra xung đột phòng học trong học kỳ hiện tại">
            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Phòng học" value={room} onChange={(event) => setRoom(event.target.value)} />
  ...
  return (
    <div className="grid gap-6">
      <PageTitleBlock
        ...
        actions={<ExportButtons fileName="academic-reports.csv" onExportCsv={() => void reportService.exportReportCsv()} />}
      />
      <div className="grid gap-4 lg:grid-cols-4">
        <StatCard label="Tổng section" value={String(rows.length)} hint="Học kỳ hiện tại" />
        <StatCard label="Section full" value={String(rows.filter((row) => row.status === 'FULL').length)} hint="Cần cân nhắc mở thêm lớp" />
        <StatCard label="Lấp đầy trung bình" value={`${Math.round((rows.reduce((sum, row) => sum + row.utilizationRate, 0) / Math.max(rows.length, 1)) * 100)}%`} hint="Trung bình toàn học kỳ" />
        <StatCard label="Dưới ngưỡng mở lớp" value={String(rows.filter((row) => row.registeredCount < Math.max(10, row.capacity * 0.3)).length)} hint="Cần theo dõi tiếp" />
      </div>
          ...
          {rows.map((row) => (
            <div key={row.id} className="rounded-2xl border border-slate-200 bg-white px-4 py-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <StatusBadge kind="section" status={row.status} />
              </div>
              <div className="mt-3 h-2 rounded-full bg-slate-100">
                <div className="h-2 rounded-full bg-teal-500" style={{ width: `${row.utilizationRate * 100}%` }} />
              </div>
              <p className="mt-2 text-xs text-slate-500">
                {row.registeredCount}/{row.capacity} sinh viên - {Math.round(row.utilizationRate * 100)}%
              </p>
            </div>
          ))}
        </div>
