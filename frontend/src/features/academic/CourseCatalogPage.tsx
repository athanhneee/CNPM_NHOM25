  ...
  return {
    ...
    actor: currentUser
      ? {
          ...
          actorRole: currentUser.roles[0] ?? "ACADEMIC_OFFICE",
        }
      : null,
  };
}
...
function buildRoomOptions(rooms: string[]) {
  return Array.from(
    new Set([...DEFAULT_ROOM_OPTIONS, ...rooms.filter(Boolean)]),
...
function WishStatusBadge({ status }: { status: WishRequest["status"] }) {
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${wishStatusClassNames[status]}`}
    >
...
export function CourseCatalogPage() {
  ...
  const rows = snapshot.courses.filter((course) => {
    ...
    const matchesSemester =
      semesterFilter === "ALL" ||
      String(course.suggestedSemester ?? "") === semesterFilter;
  ...
  if (!currentUser || !actor) {
    return (
      <EmptyState
        title="Không tìm thấy tài khoản phòng đào tạo"
  ...
  const columns: TableColumn<(typeof rows)[number]>[] = [
    ...
    },
    {
      ...
      header: "HK gợi ý",
      render: (row) =>
        row.suggestedSemester ? `HK ${row.suggestedSemester}` : "--",
    },
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`rounded-full px-3 py-1 text-sm font-semibold ${row.status === "ACTIVE" ? "bg-teal-50 text-teal-700" : "bg-slate-100 text-slate-600"}`}
        >
          {row.status}
        </span>
      ),
          ...
          <Button
            ...
            onClick={() => {
              ...
              setForm({
                ...
                majorsSupported:
                  row.majorsSupported?.join(", ") ?? "Công nghệ thông tin",
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <StatCard
          label="Tổng học phần"
          value={String(snapshot.courses.length)}
          hint="Toàn bộ catalog"
        />
        <StatCard
          label="Đang hoạt động"
          value={String(
            snapshot.courses.filter((course) => course.status === "ACTIVE")
              .length,
          )}
          hint="Có thể mở lớp học phần"
        />
        <StatCard
          label="Tạm ngưng"
          value={String(
            snapshot.courses.filter((course) => course.status === "INACTIVE")
              .length,
          )}
          hint="Tạm ẩn khỏi quy trình"
        />
        <StatCard
          label="Có điều kiện"
          value={String(
            snapshot.courses.filter(
              (course) =>
                ...
                course.prestudy.length ||
                course.corequisites.length,
            ).length,
          )}
          hint="Tiên quyết / học trước / song hành"
        />
      </div>
      ...
      <FilterBar>
        <SearchInput
          label="Tìm môn học"
          placeholder="INT2102, an toàn mạng..."
          value={query}
        ...
        <Select
          ...
          options={[
            ...
            ...majorOptions.map((m) => ({ label: m, value: m })),
          ]}
        />
        <Select
          ...
          options={[
            ...
            ...blockOptions.map((b) => ({
              label: ACADEMIC_BLOCK_LABELS[b] || b,
              value: b,
            })),
          ]}
        />
      ...
      <Drawer
        open={drawerOpen}
        title={editingId ? "Cập nhật học phần" : "Thêm học phần mới"}
      ...
      >
        <div className="grid gap-4">
          ...
          <Input
            ...
            value={form.credits}
            onChange={(event) =>
              setForm((value) => ({ ...value, credits: event.target.value }))
            }
          />
          <Input
            label="Ngành áp dụng"
            value={form.majorsSupported}
            onChange={(event) =>
              setForm((value) => ({
                ...value,
                majorsSupported: event.target.value,
              }))
            }
          />
          <Select
            ...
            onChange={(event) =>
              setForm((value) => ({
                ...value,
                courseType: event.target.value as CourseTypeValue,
              }))
            }
          ...
          <Select
            label="Khối kiến thức"
            value={form.academicBlock}
            onChange={(event) =>
              setForm((value) => ({
                ...value,
                academicBlock: event.target.value as AcademicBlockValue,
              }))
            }
          ...
          <Select
            label="Học kỳ gợi ý"
            value={form.suggestedSemester}
            onChange={(event) =>
              setForm((value) => ({
                ...value,
                suggestedSemester: event.target.value,
              }))
            }
            options={[
                ...
                try {
                  ...
                  pushToast({
                    ...
                    title: editingId
                      ...
                      : "Đã tạo học phần",
                    description: "Catalog môn học đã được đồng bộ.",
                  });
                  setDrawerOpen(false);
                } catch (error) {
                  pushToast({
                    tone: "error",
                    title: "Không thể lưu học phần",
                    description:
                      error instanceof Error
                        ? error.message
                        : "Hệ thống không thể xử lý.",
                  });
                }
              }}
              type="button"
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          try {
            ...
            pushToast({
              tone: "success",
              title: "Đã xóa mềm học phần",
              description: "Trạng thái môn học đã được cập nhật sang INACTIVE.",
            });
          } catch (error) {
            pushToast({
              tone: "error",
