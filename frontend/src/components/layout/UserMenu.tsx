...
export function UserMenu() {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        className="brand-ring interactive-press flex items-center gap-3 rounded-[24px] border border-slate-200 bg-white px-3 py-2.5 text-left shadow-sm"
        onClick={() => setOpen((value) => !value)}
        type="button"
      >
        <div className="grid h-10 w-10 place-items-center rounded-[20px] bg-gradient-to-br from-teal-500 to-cyan-500 text-sm font-semibold text-white">
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+0.75rem)] z-30 grid min-w-64 gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
          ...
          <Link
            to="/change-password"
            className="interactive-press rounded-[22px] px-3 py-3 text-sm text-slate-700 hover:bg-slate-50"
          >
            <span className="inline-flex items-center gap-2">
