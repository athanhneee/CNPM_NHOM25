...
## PROMPT 01 — Áp dụng hiệu ứng chuyển động toàn dự án

Tôi muốn áp dụng hiệu ứng tương đương class sau vào toàn bộ dự án:

`animate-in fade-in-0 slide-in-from-bottom-5 zoom-in-95 duration-500 [animation-timing-function:cubic-bezier(0.16,1,0.3,1)]`

Hãy kiểm tra dự án trước khi sửa. Dự án đang dùng Tailwind CSS v4 và hiện chưa chắc có sẵn utility `animate-in`, `fade-in-0`, `slide-in-from-bottom-5`, `zoom-in-95`. Vì vậy không được chỉ dán class rồi coi như xong.

Yêu cầu xử lý:

1. Tạo animation dùng được thật trong `frontend/src/index.css`, ví dụ:

   * opacity từ 0 về 1;
   * translateY từ khoảng 20px về 0;
   * scale từ 0.95 về 1;
   * thời lượng 500ms;
   * timing function `cubic-bezier(0.16,1,0.3,1)`;
   * có hỗ trợ `prefers-reduced-motion: reduce`.

2. Tạo class dùng chung, ví dụ:
...
3. Áp dụng hiệu ứng ở cấp route/layout thay vì gắn thủ công vào từng component:

   * Kiểm tra `frontend/src/app/router.tsx`.
   * Kiểm tra `frontend/src/components/layout/AppShell.tsx`.
   * Dùng `useLocation()` và key theo `location.pathname` hoặc tạo component `RouteTransition`.
   * Bọc phần `<Outlet />` để khi chuyển trang có hiệu ứng nhẹ.
   * Login, Forbidden, NotFound cũng cần có animation.
...
4. Áp dụng thêm cho:
   ...
   * `Drawer`;
   * Toast nếu có;
   * các panel/card quan trọng nếu phù hợp.

5. Không làm animation quá nhiều gây lag mobile.

6. Không phá layout, spacing, responsive hiện tại.

7. Không đổi logic nghiệp vụ.

8. Sau khi sửa, test trên mobile width 375px, 390px, 430px và desktop.
...
Commit message:
`feat(frontend): add global smooth route and panel animations`

---

## PROMPT 02 — Sửa lỗi frontend không build được do import thừa

Frontend hiện đang fail build/lint vì `BookOpenText` import nhưng không dùng ở `frontend/src/features/pages.tsx`.

Yêu cầu:

1. Mở `frontend/src/features/pages.tsx`.
2. Kiểm tra import `BookOpenText`.
3. Nếu không dùng thật thì xoá import.
4. Nếu có màn hình nào cần icon đó thì dùng đúng chỗ, nhưng không thêm icon chỉ để né lint.
5. Kiểm tra toàn bộ frontend còn import thừa nào khác không.
6. Không sửa logic nghiệp vụ.
...
Commit message:
`fix(frontend): remove unused icon import blocking production build`
